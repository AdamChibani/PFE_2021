...
import mainService from 'app/services/tekru/mainService';
...
function CoverImageEditor(props) {
    ...
    const handelDropFile = (file) => {
        ...
        mainService.uploadImage(file[0], 'menu-categories', elementId)
            .then((response) => {
    ...
    return (
        <div className={classes.rootDropContainer}>
            ...
            <div {...getRootProps({className: classes.dropzone})} style={{
                ...
                <em className={classes.containerEM}>(Seules les images *.jpeg et *.png seront acceptées)</em>
            </div>
        </div>
