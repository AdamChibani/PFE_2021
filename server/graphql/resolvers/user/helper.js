...
const userHelpers = {
  ...
  user: null,
  passwordToken: [],
  async getEnterpriseAdmin(requestedFields) {
    const include = gQlHelpers.getIncludesFields(requestedFields);
    return User.findAll({ where: { is_enterprise_admin: true } });
  },
  async getMyProfile(id, requestedFields) {
    const include = gQlHelpers.getIncludesFields(requestedFields);
  ...
  async update(data) {
    ...
    if (data.companyId) data.company_id = data.companyId;
    // Update and return
    return await user
      .update(data)
      .then(() => {
      ...
      include: [
        {
          ...
          as: "role",
          include: [
            { model: UsersAccess, through: UsersPrivilege, as: "accesses" },
          ],
        },
      ],
    });
  ...
  async updateUse(input) {
    const u = await User.findByPk(input.id);
    return await u.update(input);
  },
