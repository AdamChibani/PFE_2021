...
const Header = (props) => {
  const { loading, error, data } = useQuery(getCount, {
  ...
  useEffect(() => {
    if (props?.user?.user?.id) {
      gContext.setUserId(props?.user?.user?.id);
    }
  }, []);
    ...
    <>
      ...
      >
              ...
              <div className="header-btn-devider ml-auto ml-lg-5 pl-2 d-none d-xs-flex align-items-center">
                ...
                <div>
                  <Dropdown className="show-gr-dropdown py-5">
                    ...
                    >
                      {props.user.user.profileImage ? (
                        <div className="circle-40">
                          <img src={props.user.user.profileImage} alt="" />
                        </div>
                      ) : (
                        <Avatar>
                          {props.user.user.firstName[0].toUpperCase()}
                        </Avatar>
                      )}
        ...
        <Modal
          ...
          aria-describedby=""
        >
          <ConsultantInfo id={props?.user?.user?.id} />
        </Modal>
      </SiteHeader>
      <Offcanvas
